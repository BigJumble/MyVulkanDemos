...
int main()
{
  ...
  registry.emplace<Transfrom>(entity,glm::mat4x4(1.0));

  auto eview = registry.view<Transfrom>();

  for(auto [entity, transform]:eview.each())
  {
    std::println("ent {}",(uint32_t) entity);

    for ( int i = 0; i < 4; ++i )
    {
      std::println( "  [{}, {}, {}, {}]", transform[i][0], transform[i][1], transform[i][2], transform[i][3] );
    }
  }
  ...
  try
  {
    ...
    // Pause to cap the frame rate at ~60fps
